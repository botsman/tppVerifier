...
func (r *TppSqliteRepository) GetTpp(ctx context.Context, id string) (*models.TPP, error) {
	// Example query, adjust fields and table as needed
	...
	// TODO: Load tpp.Services if needed
	return tpp, nil
}

func (r *TppSqliteRepository) GetRootCertificates(ctx context.Context) ([]string, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT cert_raw FROM certs WHERE is_active = 1 AND position = ?`, "Root")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
